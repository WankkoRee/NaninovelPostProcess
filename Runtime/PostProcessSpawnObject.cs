...
namespace NaninovelPostProcess
{
    ...
    public abstract class PostProcessSpawnObject : MonoBehaviour
    {
        ...
        protected async Awaitable ChangeVolumeWeightAsync(float volumeWeight, float duration, AsyncToken asyncToken = default)
        {
            if (duration > 0) await volumeWeightTweener.Run(new FloatTween(Volume.weight, volumeWeight, new(duration), x => Volume.weight = x), asyncToken, Volume);
        ...
        public async Awaitable AwaitDestroy(AsyncToken asyncToken = default)
        {
            CompleteTweens();
